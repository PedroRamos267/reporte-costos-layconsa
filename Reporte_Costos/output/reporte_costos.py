...
def calcular_semi(codigo_semi, cantidad_req, df_exp, df_tie, cache, resumen_global):
    """
    ...
    hijos = df_exp[df_exp["Código Semi"] == str(codigo_semi)].copy()
    if hijos.empty:
        return 0, []
    ...
    for _, row in hijos.iterrows():
        ...
        if es_fabricado(familia):
            ...
            costo_calc, sub_det = calcular_semi(
                componente, cantidad, df_exp, df_tie, cache, resumen_global
            )
...
def exportar_excel(df_resumen, df_detalle):
    ...
    with pd.ExcelWriter(ruta, engine="openpyxl") as writer:
        ...
        for codigo_pt in df_resumen["Código PT"].unique():
            df_pt  = df_resumen[df_resumen["Código PT"] == codigo_pt][
                ["Tipo de Costo", "Costo Unitario", "% del Total"]
            ].copy()
            total    = df_pt["Costo Unitario"].sum()
            fila_tot = pd.DataFrame([{"Tipo de Costo": "TOTAL",
                                       "Costo Unitario": total, "% del Total": 1.0}])
            df_pt    = pd.concat([df_pt, fila_tot], ignore_index=True)
            df_pt["% del Total"]    = df_pt["% del Total"].map("{:.1%}".format)
            df_pt["Costo Unitario"] = df_pt["Costo Unitario"].map("{:.6f}".format)
            df_pt.to_excel(writer, sheet_name=str(codigo_pt)[:31], index=False)

        resumen_global = df_resumen.groupby(
            ["Código PT", "Descripción PT", "Tipo de Costo"]
        ).agg(Costo_Unitario=("Costo Unitario", "sum")).reset_index()
        ...
        default_fill = PatternFill("solid", fgColor="1F5C2E")

        for sheet_name, ws in writer.sheets.items():
            fill = fills.get(sheet_name, default_fill)
            for cell in ws[1]:
...
def lanzar_dashboard(df_resumen, df_detalle):
    try:
    ...
    def actualizar(codigo_pt):
        ...
        def color_tipo(t):
            if t.startswith("CM"):    return COLORES["CM"]
            elif t.startswith("MOD"): return COLORES["MOD"]
            elif t.startswith("CIF"): return COLORES["CIF"]
            return COLORES["TOTAL"]

        colores_wf = [color_tipo(t) for t in df_pt["Tipo de Costo"]] + [COLORES["TOTAL"]]
        ...
        return (kpis_elem, fig_cas, fig_cas_pct, fig_don,
                ...
                df_det_fmt.to_dict("records"), cols_det)
...
def main():
    ...
    if opcion in ("2", "3"):
        lanzar_dashboard(df_resumen, df_det)
